...
//			L.debug("nettask","随机数结果集add : "+num);
		}
		int setSize = set.size();
//		L.debug("nettask","随机数结果集 : "+setSize);
		// 如果存入的数小于指定生成的个数，则调用递归再生成剩余个数的随机数，如此循环，直到达到指定大小
		if (setSize < 10) {
//			L.debug("nettask","setSize : "+setSize+"   n : "+n+"   n - setSize : "+(n - setSize));
			randomSet(min, max, 10 - setSize, set);// 递归
		}
	}
